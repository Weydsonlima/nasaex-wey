...
export const createWorldAsset = base
  ...
  .handler(async ({ input, context, errors }) => {
    if (context.user.role !== "ADMIN") throw errors.FORBIDDEN({ message: "Apenas admins podem criar assets" });
...
export const updateWorldAsset = base
  ...
  .handler(async ({ input, context, errors }) => {
    if (context.user.role !== "ADMIN") throw errors.FORBIDDEN({ message: "Apenas admins podem editar assets" });
    const { id, ...data } = input;
...
export const deleteWorldAsset = base
  ...
  .handler(async ({ input, context, errors }) => {
    if (context.user.role !== "ADMIN") throw errors.FORBIDDEN({ message: "Apenas admins podem deletar assets" });
