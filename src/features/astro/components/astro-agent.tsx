...
import {
  X, Send, Minimize2, Maximize2, RotateCcw, CheckCircle2,
} from "lucide-react";
...
function getContext(history: Message[]) {
  ...
  const lastInstallMsg = [...history].reverse().find(
    (m) => m.role === "user" && /^Instalar .+/.test(m.text) && !m.text.startsWith("Instalar outra") && !m.text.startsWith("Instalar nova"),
  );
  ...
  const recentContext = history.slice(-8).map((m) => m.text.toLowerCase()).join(" ");
...
function buildAstroResponse(
  ...
  if (
    ...
    q.includes("necessidade") || q.includes("sistematização") ||
    ...
    return [{
      ...
      quickReplies: [
        { label: "🎯 N — Necessidade (Chat)", value: "como usar o chat", link: "/chat" },
        { label: "📊 A — Análise (Insights)", value: "como usar insights", link: "/insights" },
        { label: "⚙️ S — Sistematizar (Integrações)", value: "quero instalar uma integração", link: "/integrations" },
        { label: "🔥 A — Ação (FORGE)", value: "como usar o forge", link: "/forge" },
      ],
    }];
  }
  ...
  if (
    ...
    q.includes("como configuro") || q.includes("como configur") ||
    q.includes("setup") || q.includes("ativar integração")
  ) {
    if (lastInstalledIntegration) {
      return [{
        ...
        quickReplies: [
          ...
          { label: "🔌 Instalar outra integração", value: "quero instalar uma integração" },
        ],
      }];
    }
    return [{
  ...
  // ── FORGE ─────────────────────────────────────────────────────────────────
  if (q.includes("forge") || q.includes("proposta") || q.includes("contrato") || q.includes("pagamento link")) {
    return [{
      id: id(),
      role: "astro",
      ...
      quickReplies: [
        ...
        { label: "💳 Configurar gateway de pagamento", value: "configurar pagamento forge" },
        { label: "📄 Criar padrão de contrato", value: "criar padrão contrato" },
  ...
  if (q.includes("tracking") || q.includes("lead") || q.includes("pipeline") || q.includes("funil") || q.includes("oportunidade")) {
    return [{
      id: id(),
      role: "astro",
      ...
      quickReplies: [
        ...
        { label: "🤖 Criar automação de lead", value: "como criar automação" },
        { label: "🔌 Integrar WhatsApp ao CRM", value: "instalar whatsapp business" },
        { label: "🚀 Método NASA", value: "o que é o método nasa" },
      ],
    }];
  }
  ...
  // ── Automação ─────────────────────────────────────────────────────────────
  if (q.includes("automação") || q.includes("automation") || q.includes("workflow") || q.includes("fluxo")) {
    return [{
      id: id(),
      role: "astro",
      ...
      quickReplies: [
        ...
        { label: "💬 Enviar WhatsApp automático", value: "instalar whatsapp business" },
        { label: "📧 Automatizar e-mail", value: "instalar marketing" },
        { label: "⚙️ Ver Integrações", value: "listar integrações", link: "/integrations" },
      ],
    }];
  }

  // ── Insights / Analytics ──────────────────────────────────────────────────
  if (q.includes("insight") || q.includes("relatório") || q.includes("dashboard") || q.includes("métrica") || q.includes("análise")) {
    return [{
      id: id(),
      role: "astro",
  ...
  // ── Agenda / Agendamentos ──────────────────────────────────────────────────
  if (q.includes("agenda") || q.includes("agendamento") || q.includes("calendário") || q.includes("horário")) {
    return [{
      id: id(),
      role: "astro",
      ...
      quickReplies: [
        ...
        { label: "📅 Integrar Google Calendar", value: "instalar google workspace" },
  ...
  // ── Chat / Atendimento ────────────────────────────────────────────────────
  if (q.includes("chat") || q.includes("atendimento") || q.includes("conversa") || q.includes("inbox")) {
    return [{
      id: id(),
      role: "astro",
      ...
      quickReplies: [
        ...
        { label: "📱 Instalar WhatsApp Business", value: "instalar whatsapp business" },
  ...
  if (q.includes("funcionalidade") || q.includes("recurso") || q.includes("o que é") || q.includes("sobre o nasa") || q.includes("início") || q.includes("inicio")) {
    return [{
      id: id(),
      role: "astro",
      ...
      quickReplies: [
        ...
        { label: "🔌 Integrações", value: "listar integrações", link: "/integrations" },
      ],
    }];
  }
...
function MessageBubble({
  ...
  return (
    <div className={cn("flex gap-2", isAstro ? "items-start" : "items-start flex-row-reverse")}>
      {isAstro && (
        <div className="size-7 rounded-full bg-gradient-to-br from-[#7C3AED] to-[#a855f7] flex items-center justify-center shrink-0 text-sm shadow-sm mt-0.5">
          👨‍🚀
        </div>
      )}

      <div className={cn("space-y-2 max-w-[82%]", !isAstro && "items-end flex flex-col")}>
        <div className={cn(
          ...
          <p className="whitespace-pre-line text-[13px]">{formatText(msg.text)}</p>
        </div>
            ...
            <div className="flex items-center gap-2">
              {msg.integration.icon.startsWith("http") && !imgError ? (
                <div className="size-8 rounded-lg overflow-hidden bg-white border flex items-center justify-center">
                  ...
                  <img src={msg.integration.icon} alt={msg.integration.name}
                    className="size-6 object-contain" onError={() => setImgError(true)} />
                </div>
              ) : (
                <span className="text-lg">{msg.integration.icon.startsWith("http") ? "🔌" : msg.integration.icon}</span>
              )}
              <div>
                <p className="text-xs font-semibold">{msg.integration.name}</p>
                <p className="text-[10px] text-muted-foreground">{CATEGORY_LABELS[msg.integration.category]}</p>
              </div>
...
export function AstroAgent() {
  const { astroOpen, setAstroOpen, pendingInstall, clearPendingInstall, install, isInstalled } = useMarketplace();
      ...
      if (integration) {
        if (isInstalled(slug)) {
          setMessages((prev) => [...prev,
            {
              id: generateId(), role: "astro",
              text: `**${integration.name}** já está instalado! ✅`,
              quickReplies: [
                { label: "⚙️ Configurar", value: `configurar credenciais`, link: `/integrations/${slug}` },
        ...
        setMessages((prev) => [...prev,
          { id: generateId(), role: "user", text: `Instalar ${integration.name}` },
        ]);
        setThinking(true);
        ...
        setMessages((prev) => [...prev,
          {
            ...
            quickReplies: [
              { label: `⚙️ Inserir credenciais`, value: `configurar credenciais`, link: `/integrations/${slug}` },
              { label: "❓ O que preciso inserir?", value: "o que é api key" },
              { label: "🔌 Instalar outra integração", value: "quero instalar uma integração" },
            ],
          },
        ]);
  ...
  const lastAstroIndex = [...messages].map((m, i) => ({ m, i })).reverse().find(({ m }) => m.role === "astro")?.i ?? -1;
    ...
    <>
      ...
      <button
        ...
        className={cn(
          ...
          "bg-gradient-to-br from-[#7C3AED] to-[#a855f7] hover:scale-110 hover:shadow-[0_0_24px_rgba(124,58,237,0.5)]",
        )}
      >
      ...
      {astroOpen && (
        ...
        >
          {/* Header */}
          <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-[#7C3AED] to-[#a855f7] text-white shrink-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-white/70 mt-0.5">Assistente Inteligente NASA</p>
            </div>
            <div className="flex items-center gap-1">
              <Button
                variant="ghost" size="icon" className="size-7 text-white/70 hover:text-white hover:bg-white/10"
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3 h-[426px]">
                {messages.map((msg, index) => (
                  <div key={msg.id}>
                    ...
                    <MessageBubble
                      msg={index === lastAstroIndex ? msg : { ...msg, quickReplies: undefined }}
                ...
                {thinking && (
                  <div className="flex gap-2 items-start">
                    <div className="size-7 rounded-full bg-gradient-to-br from-[#7C3AED] to-[#a855f7] flex items-center justify-center text-sm shrink-0">
                      👨‍🚀
                    </div>
