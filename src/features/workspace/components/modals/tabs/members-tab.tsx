...
export function MembersTab({ workspaceId }: { workspaceId: string }) {
  const { members: currentMembers, isLoading } = useWorkspaceMembers(workspaceId);
  const addMember = useAddWorkspaceMember();
  const removeMember = useRemoveWorkspaceMember();

  const { data: orgData } = useQuery(
    orpc.orgs.listMembers.queryOptions({
      input: { query: { userIds: currentMembers?.map((m: any) => m.user.id) || [] } },
    }),
  );
    ...
    <div className="max-w-2xl space-y-8">
      <div className="flex items-center justify-between">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button size="sm">
              <UserPlus className="size-4 mr-2" />Convidar Membro
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-80 p-0" align="end">
            <div className="p-3 border-b">
              <h4 className="font-medium text-sm">Membros da Organização</h4>
              <p className="text-xs text-muted-foreground">Selecione para adicionar ao workspace</p>
            </div>
            <div className="max-h-[300px] overflow-y-auto p-1">
              {availableMembers.map((user: any) => (
                <button
                  key={user.id}
                  onClick={() => addMember.mutate({ workspaceId, userId: user.id })}
              ...
              {availableMembers.length === 0 && (
                <div className="p-4 text-center text-xs text-muted-foreground">Todos os membros já estão no workspace.</div>
              )}
            </div>
          </PopoverContent>
        </Popover>
      ...
      <div className="space-y-4">
        {currentMembers.map((member: any) => (
          <div key={member.id} className="flex items-center justify-between p-3 border rounded-lg bg-background group">
            ...
            <div className="flex items-center gap-4">
              ...
              {member.role !== "OWNER" && (
                <Button
                  ...
                  onClick={() => {
                    if (confirm(`Deseja remover ${member.user.name} deste workspace?`)) {
