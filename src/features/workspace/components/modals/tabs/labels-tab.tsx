    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="flex items-center gap-3 p-3 border rounded-lg bg-muted/30">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Input type="color" value={newColor} onChange={(e) => setNewColor(e.target.value)} className="size-7 p-0.5 cursor-pointer w-7" />
        </div>
        <Input
        ...
        <Button size="sm" onClick={handleCreate} disabled={!newName.trim() || createTag.isPending}>
          <Plus className="size-4 mr-1" />Criar
        </Button>
      </div>
            ...
            {editingId === tag.id ? (
              <>
                <div className="flex items-center gap-1.5 flex-wrap">
                  {PRESET_COLORS.map((c) => (
                    <button key={c} className={cn("size-4 rounded-full", editColor === c && "ring-2 ring-offset-1 ring-foreground")} style={{ backgroundColor: c }} onClick={() => setEditColor(c)} />
                  ))}
                  <Input type="color" value={editColor} onChange={(e) => setEditColor(e.target.value)} className="size-6 p-0.5 cursor-pointer w-6" />
                </div>
                <Input value={editName} onChange={(e) => setEditName(e.target.value)} className="flex-1 h-8" autoFocus onKeyDown={(e) => e.key === "Enter" && handleSaveEdit()} />
                <Button size="icon" variant="ghost" className="size-8 text-emerald-600" onClick={handleSaveEdit}><Check className="size-4" /></Button>
              ...
              <>
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <Button size="icon" variant="ghost" className="size-7" onClick={() => { setEditingId(tag.id); setEditName(tag.name); setEditColor(tag.color); }}><Pencil className="size-3.5" /></Button>
                  <Button size="icon" variant="ghost" className="size-7 text-destructive hover:bg-destructive/10" onClick={() => confirm(`Excluir etiqueta "${tag.name}"?`) && deleteTag.mutate({ tagId: tag.id })}><Trash2 className="size-3.5" /></Button>
                </div>
              </>
            )}
          </div>
