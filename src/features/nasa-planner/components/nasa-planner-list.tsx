...
import {
  ...
  Trash2Icon, RocketIcon, BuildingIcon, FolderIcon, ChevronRightIcon,
} from "lucide-react";
...
import { useNasaPlanners, useCreatePlanner, useDeletePlanner } from "../hooks/use-nasa-planner";
      ...
      <div className="flex items-center justify-between px-6 py-5 border-b">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Planejamento estratégico de marketing com IA</p>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-muted-foreground text-sm mt-1">Crie seu primeiro planner de marketing associando uma empresa e projeto/cliente</p>
            </div>
            <Button onClick={() => setCreateOpen(true)} className="gap-2">
                  ...
                  <div className="space-y-1">
                    {planner.clientOrgName && (
                      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        ...
                        <span className="truncate">{planner.clientOrgName}</span>
                      </div>
                    )}
                    {planner.orgProject && (
                      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        <div className="size-2.5 rounded-full shrink-0" style={{ backgroundColor: planner.orgProject.color ?? "#7c3aed" }} />
                        <span className="truncate">{planner.orgProject.name}</span>
                      </div>
                    )}
                  </div>

                  {planner.description && (
                    <CardDescription className="text-xs line-clamp-2 mt-1">{planner.description}</CardDescription>
                  )}
                </CardHeader>
                <CardContent>
      ...
      <Dialog open={createOpen} onOpenChange={(v) => { setCreateOpen(v); if (!v) resetForm(); }}>
        <DialogContent className="max-w-md">
                      ...
                      <CommandList>
                        ...
                        <CommandGroup>
                          ...
                          {orgProjects.map((p: any) => (
                            <CommandItem key={p.id} value={p.name} onSelect={() => { setSelectedProjectId(p.id); setProjectOpen(false); }}>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreate} disabled={!form.name.trim() || !selectedOrgId || createPlanner.isPending}>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Todos os posts, mapas mentais e campanhas deste planner serão excluídos permanentemente.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
