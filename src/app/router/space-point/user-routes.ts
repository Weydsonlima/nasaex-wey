...
import { ensureLevelsSeed, ensureOrgRules, ensureUserPoint, awardPoints, periodToDateRange, getBadgeUrlMap, resolveBadgeUrl } from "./utils";
...
export const earnSpacePoints = base
  ...
  .output(z.object({ awarded: z.number(), totalPoints: z.number(), newSeals: z.array(z.object({ name: z.string(), badgeNumber: z.number(), planetEmoji: z.string(), badgeUrl: z.string() })), popupTemplateId: z.string().nullable() }))
  .handler(async ({ input, context }) => {
    const { user, session } = context;
    const orgId = session.activeOrganizationId;
    if (!orgId) return { awarded: 0, totalPoints: 0, newSeals: [], popupTemplateId: null };
    const result = await awardPoints(user.id, orgId, input.action, input.description, input.metadata as object);
    return { awarded: result.points, totalPoints: result.totalPoints, newSeals: result.newSeals, popupTemplateId: result.popupTemplateId };
  });
...
export const updateSpacePointRule = base
  ...
  .input(z.object({ id: z.string(), points: z.number().optional(), cooldownHours: z.number().nullable().optional(), isActive: z.boolean().optional(), label: z.string().optional() }))
...
export const createSpacePointRule = base
  ...
  .input(z.object({ action: z.string().min(1), label: z.string().min(1), points: z.number(), cooldownHours: z.number().nullable().optional() }))
