...
export const adminGetOrgRules = base
  ...
  .handler(async ({ input }) => {
    const rules = await prisma.spacePointRule.findMany({ where: { orgId: input.orgId }, orderBy: [{ points: "desc" }, { label: "asc" }] });
    ...
    const defaultRuleMap = Object.fromEntries(DEFAULT_RULES.map((r) => [r.action, r.category]));
    return rules.map((r) => ({
...
export const adminCreateOrgRule = base
  ...
  .input(z.object({ orgId: z.string(), action: z.string().min(1), label: z.string().min(1), points: z.number(), cooldownHours: z.number().nullable().optional(), popupTemplateId: z.string().nullable().optional() }))
...
export const adminUpdateOrgRule = base
  ...
  .input(z.object({ id: z.string(), points: z.number().optional(), cooldownHours: z.number().nullable().optional(), isActive: z.boolean().optional(), label: z.string().optional(), popupTemplateId: z.string().nullable().optional() }))
